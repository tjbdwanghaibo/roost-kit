...
var (
	ErrRoomManagerNotRunning = errors.New("sync: room manager is not running")
	ErrRoomManagerStopped    = errors.New("sync: room manager is stopped")
	ErrRoomLimit             = errors.New("sync: room capacity exceeded")
	ErrRoomAlreadyExists     = errors.New("sync: room already exists")
	ErrRoomNotFound          = errors.New("sync: room not found")
	ErrRoomClosing           = errors.New("sync: room is closing")
)
...
func NewRoomManager(config RoomManagerConfig) (*RoomManager, error) {
	...
	if config.ReplicationInterval <= 0 {
		config.ReplicationInterval = DefaultRoomReplicationInterval
	}
	if config.IdleTTL <= 0 {
...
func (m *RoomManager) Create(roomID int64) (*RoomReplication, error) {
	if m == nil || roomID <= 0 {
	...
	room, err := NewRoomReplication(roomID, m.config.Downstream, RoomReplicationConfig{
...
func (m *RoomManager) Get(roomID int64) (*RoomReplication, bool) {
	if m == nil || roomID <= 0 {
		return nil, false
	}
	m.mu.RLock()
...
func (m *RoomManager) GetOrCreate(roomID int64) (*RoomReplication, error) {
...
func roomCapacityProduct(left, right int) (int, error) {
	...
	if left <= 0 || right <= 0 || left > maxInt/right {
		return 0, fmt.Errorf("sync: room capacity product overflows: %d * %d", left, right)
	}
