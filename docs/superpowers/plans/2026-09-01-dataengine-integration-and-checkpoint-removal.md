...
- [ ] **Step 7: 运行脚本测试和真实 up/status/down/up**
...
- [ ] **Step 3: 实现 fixture，并仅补必要的 Mod 可测试入口**
...
- [ ] **Step 4: 实现真实 transaction、patch 和 load/migrate 断言**
...
- [ ] **Step 1: 写 NATS outage 和 Mongo primary failover 失败测试**
...
- [ ] **Step 2: 运行并确认 fault helpers 不存在而失败**
...
- [ ] **Step 1: 写 architecture/import 和 CombatDao mutation 失败测试**
...
- [ ] **Step 2: 运行并确认旧 import/dirty 行为导致失败**
...
- [ ] **Step 1: 写禁止 Checkpoint package/legacy symbols 的失败测试**
...
- [ ] **Step 4: 运行全量 core/skill 测试和引用扫描**
...
- [ ] **Step 1: 将 persistence tests 改为只接受 Data Engine，并确认失败**
...
- [ ] **Step 2: 简化 persistence selection 和模块依赖**
...
- [ ] **Step 3: 删除 legacy runtime files 并更新 contract tests/docs**
...
- [ ] **Step 4: 运行 kit 全量、race、vet 和真实门禁**
...
- [ ] **Step 1: 更新 generator tests，先要求无 legacy 输出**
...
- [ ] **Step 2: 执行 race、vet、真实门禁和 architecture scan**
