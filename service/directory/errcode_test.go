...
var codedSentinels = map[int32]error{
	CodeKeyTaken:      ErrKeyTaken,
	CodeClaimNotFound: ErrClaimNotFound,
	CodeClaimStale:    ErrClaimStale,
	CodeOwnerMismatch: ErrOwnerMismatch,
	CodeKeyEmpty:      ErrKeyEmpty,
	CodeOwnerEmpty:    ErrOwnerEmpty,
}
...
// the internal code. A release that lost its compare-and-set means someone else holds the key now.
func TestAForeignSentinelIsMappedIntoThisSegment(t *testing.T) {
	err := fmt.Errorf("update: %w", versionstore.ErrVersionMismatch)
	code := Code(err)
	...
	if code != CodeOwnerMismatch {
		t.Fatalf("%v maps to %d, want %d", err, code, CodeOwnerMismatch)
	}
}
